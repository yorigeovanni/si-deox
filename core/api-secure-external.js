...
async function applyResponseInterceptor(response, logoutUserExternal) {
  if (!response.ok) {
...
async function fetchRequest(method, baseURL, url, data, config = {}) {
  ...
  const { logoutUserExternal, headers, ...restConfig } = config;
  ...
  const finalResponse = await applyResponseInterceptor(response, logoutUserExternal);
  try {
