...
const query_keys = ["public-informasi"];
//===================================================================================
  ...
  const { 
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        const { data : { records, length } } = await post('/mobile/api/portal/informasi',{
          offset: pageParam
        });
        return {
          data: records || [],
...
function ItemCard({ item_record }) {
  ...
  const { mutate, isError, error, isPending } = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await post(`/mobile/api/portal/informasi/like`,{
        id : item_record.id,
      });
      return result;
    },
