...
const query_keys = ["public-kegiatan"];
//===============================================================================
  ...
  const { 
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        const { data : { records, length } } = await post('/mobile/api/portal/kegiatan',{
          offset: pageParam
        });
        return {
          data: records || [],
...
function ItemCard({ item_record }) {
  ...
  const { mutate, isError, error, isPending } = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await post(`/mobile/api/portal/kegiatan/like`,{
        id : item_record.id,
      });
      return result;
    },
