import { View, Text, TouchableOpacity, RefreshControl, ActivityIndicator, Animated, StyleSheet, Dimensions, FlatList, Alert, Modal } from "react-native";
...
const query_keys = ["public-fasilitas"];
//===================================================================================
  ...
  const { 
    ...
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        const { data : { records, length } } = await post('/mobile/api/portal/fasilitas',{
          offset: pageParam
        });
        return {
          data: records || [],
...
function ItemCard({ item_record }) {
  ...
  const { mutate, isError, error, isPending } = useMutation({
    mutationFn: async (data) => {
      const { data: result } = await post(`/mobile/api/portal/fasilitas/like`,{
        id : item_record.id,
      });
      return result;
    },
  ...
  const handleComment = useCallback(async () => {
    router.push(`/fasilitas/${item_record.id}/comments`);
  }, [item_record, router]);
        ...
        <View className="px-4 py-3 border-t border-gray-100">
                <View className="flex-row items-center justify-between mb-3">
                  <View className="flex-row items-center">
                    {item_record.isLike && (
                      <Fragment>
                        ...
                        <Text className="text-gray-600 ml-2">Thank You</Text>
                      </Fragment>
                    )}
                  </View>
                  ...
                  <View className="flex-row items-center space-x-4">
                    ...
                    <Text className="text-gray-600 ml-2">{item_record.comments} comments</Text>
                  </View>
                </View>
