...
export default function PpidInformasiBerkala() {
    ...
    return (
        <View className=' flex-1 bg-white'>
            {isLoading ? <Text>Loading...</Text> : isError ? <Text>Error: {error?.response?.data?.message || error?.message}</Text> : (<ScrollView className='mt-4 '>
                <TableItems
                    title='A. INFORMASI DAN PENGADUAN'
                    items={data.x_studio_inormasi_dan_pengaduan}
                />
                <TableItems
                    title='B. RENCANA KERJA ANGGARAN'
                    items={data.x_studio_rencana_kerja_anggaran}
                />

                <TableItems
                    title='C. KEPEGAWAIAN & KEUANGAN'
                    items={data.x_studio_rencana_kerja_anggaran}
                />

                <TableItems
                    title='D. DOKUMENT BARANG & JASA - PERENCANAAN'
                    items={data.x_studio_rencana_kerja_anggaran}
                />

                <TableItems
                    title='E. DOKUMENT BARANG & JASA - PEMILIHAN'
                    items={data.x_studio_rencana_kerja_anggaran}
                />

                <TableItems
                    title='F. DOKUMENT BARANG & JASA - PELAKSANAN'
                    items={data.x_studio_rencana_kerja_anggaran}
                />
            </ScrollView>)}
        </View>
...
function TableItems({ title, items = [] }) {
    ...
    if (items && items.length > 0) {
        ...
        return (<View className='my-4 flex-col' >
            <Text className='font-bold text-start mx-2 mb-4  text-red-600'>{title}</Text>
            <View className='flex-row items-center justify-center h-36 bg-red-200 mx-2 rounded-lg'>
