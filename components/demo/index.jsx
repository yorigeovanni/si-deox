import { View, Text, TouchableOpacity, ScrollView, Dimensions, Animated, TextInput, ActivityIndicator, RefreshControl, Alert } from "react-native";
...
export default function DemoFlightInformation() {
  ...
  const loadPenerbangan = async (
    ...
    try {
      ...
      let url = 'https://n8nxy.deoairport.co.id/webhook/fids/departures';
      if (type === "arrivals") {
        url = 'https://n8nxy.deoairport.co.id/webhook/fids/arrivals';
      }
      const { data } = await axios.get(url);
      if (!data.error) {
        setFlightData((oldState) => {
          return {
            ...oldState,
            text_tanggal: data.text_tanggal,
            total_hari_ini: data.total_hari_ini,
      ...
      console.log(e)
      if (!isAutoRefresh) {
        setError("Unable to update flight information. Please check your connection.");
      }
    } finally {
      if (!isAutoRefresh) {
        setLoading(false);
                ...
                <View className="flex-row items-center justify-between mb-4">
                  <View className="flex-1">
                    ...
                    <Text className="text-lg font-semibold mt-1">
                      {activeTab === "departures" ? flight.actual_local ? flight.actual_local : flight.jadwal_local : null}
                  ...
                  <View className="flex-1 items-end">
                    ...
                    <Text className="text-lg font-semibold mt-1">
                      {activeTab === "arrivals" ? flight.actual_local ? flight.actual_local : flight.jadwal_local : null}
                    </Text>
                  </View>
                </View>
